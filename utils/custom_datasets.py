...
DATASETS = ['writing', 'english', 'german', 'pubmed', 'guardian', 'medical', 'job_posts']
...
def load_writing(cache_dir=None, seed_val=0):
...
def load(name, cache_dir, seed_val):
    if name in DATASETS:
        load_fn = globals()[f'load_{name}']
        return load_fn(cache_dir=cache_dir, seed_val=seed_val)
    else:
...
def sample_beginning_text(sample, min_words=55, max_words=200):
    ...
    # Minus one, as we want to choose the specific item
    chosen_length = np.random.randint(min_words, max_range) if max_range > min_words else max_range
    point_chosen_end = point_chosen_begin + chosen_length

    return ' '.join(sample[point_chosen_begin : point_chosen_end])


def load_job_posts(cache_dir=None,seed_val=0, min_words=55, max_words=200):
    job_posts_path = '/home/scur1741/dataTMP/monster_com-job_sample.csv'    
    data = pd.read_csv(job_posts_path)
    data = data.dropna(subset=['job_description'])

    data["sampled"] = data["job_description"].apply(lambda x: sample_beginning_text(x, min_words, max_words))
    data = data.dropna(subset=['sampled'])
...
def load_guardian(cache_dir=None,seed_val=0, min_words=55, max_words=200):
    # guardian_path = '/home/scur1741/detectgptOriginal/data/guardian_articles.csv'
    guardian_path = '/home/scur1741/dataTMP/guardian_articles.csv'
    data = pd.read_csv(guardian_path)
    data = data.dropna()

    data["sampled"] = data["bodyContent"].apply(lambda x: sample_beginning_text(x))
    data = data.dropna()
...
def load_medical(cache_dir=None,seed_val=0, min_words=55, max_words=200):
    medical_path = '/home/scur1741/dataTMP/train.dat'
    data = pd.read_csv(medical_path, sep='\t', header=None)
    data.rename(columns = {0: 'conditions', 1: 'full_text'}, inplace=True)

    data = data.dropna()

    data["sampled"] = data["full_text"].apply(lambda x: sample_beginning_text(x))
    data = data.dropna()
