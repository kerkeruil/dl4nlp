...
from .likelihood import get_lls, get_ll
from .metric import get_roc_metrics, get_precision_recall_metrics
from utils.load_models_tokenizers import load_mask_model, load_base_model
...
def replace_masks(args, config, texts):
    DEVICE = args.DEVICE
...
def perturb_texts_(args, config, texts, span_length, pct, ceil_pct=False):
    ...
    if not args.random_fills:
        ...
        if args.random_fills_tokens:
            ...
            FILL_DICTIONARY = config["FILL_DICTIONARY"]
            masked_texts = [tokenize_and_mask(args, x, span_length, pct, ceil_pct) for x in texts]
...
def perturb_texts(args, config, texts, span_length, pct, ceil_pct=False):
    ...
    for i in tqdm.tqdm(range(0, len(texts), chunk_size), desc="Applying perturbations"):
        outputs.extend(perturb_texts_(args, config, texts[i:i + chunk_size], span_length, pct, ceil_pct=ceil_pct))
    return outputs

def get_perturbation_results(args, config, data, span_length=10, n_perturbations=1, n_samples=500):
    n_perturbation_rounds = config["n_perturbation_rounds"]
    ...
    perturb_fn = functools.partial(perturb_texts, args=args, config=config, span_length=span_length, pct=args.pct_words_masked)

    p_sampled_text = perturb_fn(texts = [x for x in sampled_text for _ in range(n_perturbations)])
    p_original_text = perturb_fn(texts = [x for x in original_text for _ in range(n_perturbations)])
    for _ in range(n_perturbation_rounds - 1):
        try:
            p_sampled_text, p_original_text = perturb_fn(texts = p_sampled_text), perturb_fn(texts = p_original_text)
    ...
    for res in tqdm.tqdm(results, desc="Computing log likelihoods"):
        p_sampled_ll = get_lls(args, config, res["perturbed_sampled"])
        p_original_ll = get_lls(args, config, res["perturbed_original"])
        res["original_ll"] = get_ll(args, config, res["original"])
        res["sampled_ll"] = get_ll(args, config, res["sampled"])
...
def detectGPT(args, config, data):
    n_samples = args.n_samples
    n_perturbation_list = config["n_perturbation_list"]
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            perturbation_results = get_perturbation_results(args, config, data, args.span_length, n_perturbations, n_samples)
            for perturbation_mode in ['d', 'z']:
                output = run_perturbation_experiment(
                    args, perturbation_results, perturbation_mode, span_length=args.span_length, n_perturbations=n_perturbations, n_samples=n_samples)
