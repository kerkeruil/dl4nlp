...
DATASETS = ['writing', 'english', 'german', 'pubmed', 'guardian', 'job_posts']
...
def sample_beginning_text(sample, min_words=55, max_words=200):
    ...
    print(min_words, max_range, "check minwords/maxrange")
...
def load_job_posts(cache_dir=None, min_words=55, max_words=200):
    job_posts_path = '/home/scur1741/detectgptOriginal/data/monster_com-job_sample.csv'
    ...
    print(data, "check data2")

    data["sampled"] = data["job_description"].apply(lambda x: sample_text(x))

    print(data, "check data3")

    data["sampled"] = data["sampled"].apply(lambda x: process_spaces(x))

    print(data, "check data4")
