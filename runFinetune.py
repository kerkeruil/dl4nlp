...
# Tokenize function with labels for causal language modeling
def tokenize_function(examples, **kwargs):
...
def set_training_args(args):
    ...
    output_dir='./resultsTraining',          # Output directory
    num_train_epochs=args.epoch,              # Number of training epochs def 10
    save_strategy="no",              # No save during training
...
def partial_train(model_name, data):
    ...
    "max_length": 256               # Optional max_length argument
    })
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="guardian", help = "The dataset you want to run your experiments on. Natively supported: XSum, PubMedQA, WritingPrompts, SQuAD, English and German splits of WMT16.")
    parser.add_argument('--dataset_for_evaluation', type=list, default=["guardian","job_posts", "medical"], help = "The dataset you want to run your experiments on. Natively supported: XSum, PubMedQA, WritingPrompts, SQuAD, English and German splits of WMT16.")
    ...
    parser.add_argument('--mask_filling_model_name', type=str, default="t5-small", help = "Model to use for filling the masks.")
    ...
    parser.add_argument('--fixed_gen_model', type=str, default= "gpt2-medium")
    parser.add_argument('--epoch', type=int, default=5)
    args = parser.parse_args()

    # for datasetEval in args.dataset_for_evaluation:
        

    #     #This is where we start feeding seed per run of the experiment.
    #     args.evalDataName = datasetEval


    # args.seed_value = i
    import copy
    original_dataset = copy.deepcopy(args.dataset)
    ...
    train_data, _ = generate_data(args, config)
    ...
    # After using training data and training model, we evaluate with all 3 test data from our datasets.
    for datasetEval in args.dataset_for_evaluation:
        for i in range(args.num_experiments):
            args.seed_value = i

            # MAKE SURE config["base_model"] = partial_train(args.scoring_model_name, train_data)
            # still stays the same
            args.dataset = datasetEval
            # config = {}
            initial_setup(args, config)
            # set_experiment_config(args, config)
            ...
            print(f"we are about the evaluate model {args.scoring_model_name} fine-tuned on {original_dataset} on dataset {args.dataset}")
            _, test_data = generate_data(args, config)
            print(test_data, )
